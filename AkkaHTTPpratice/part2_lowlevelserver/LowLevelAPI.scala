package AkkaHTTPpratice.part2_lowlevelserver
...
object LowLevelAPI extends App {
  ...
  // only Server Binding script
  val serverSource = Http().bind("localhost", 8061)
  ...
  serverBindingFuture.onComplete {
    case Success(binding) =>
     ...
     // binding.terminate(2 seconds) --terminate server
      StdIn.readLine()
    case Failure(ex) => println(s"Server binding failed: $ex")
  }

  // Server bind and request/responses script
  /*
  ...
  // shorthand version:
   Http().bindAndHandleSync(requestHandler, "localhost", 8062)
  ...
  // shorthand version
  Http().bindAndHandleAsync(asyncRequestHandler, "localhost", 8063)
  ...
  // shorthand version
  Http().bindAndHandle(streamsBasedRequestHandler, "localhost", 8064)
  ...
  val bindingFuture = Http().bindAndHandleSync(syncExerciseHandler, "localhost", 8065)
  StdIn.readLine()
  // shutdown the server:
  bindingFuture
